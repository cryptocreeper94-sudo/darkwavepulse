...
var server = http.createServer((req, res) => {
  ...
  if (urlPath === "/") {
    ...
    const isHealthCheck = !accept || accept === "*/*" || accept.includes("application/json") || userAgent.includes("GoogleHC") || userAgent.includes("kube-probe") || userAgent.includes("curl") || userAgent.includes("python") || userAgent.includes("Go-http");
    if (isHealthCheck) {
      res.writeHead(200, { "Content-Type": "application/json" });
      res.end('{"status":"ok"}');
      return;
...
function startWorkers() {
  ...
  setTimeout(async () => {
    ...
    if (!orbitRegistered) {
      setTimeout(() => registerWithOrbit(), 3e4);
    }
  }, 5e3);
...
var SSO_SECRET = process.env.SSO_JWT_SECRET || "";
var DARKWAVE_API_SECRET = process.env.DARKWAVE_API_SECRET || "";
var SSO_TOKEN_TTL = 5 * 60;
var SSO_SESSION_TTL = 24 * 60 * 60;
var ALLOWED_SOURCE_APPS = ["pulse", "darkwavestudios", "orbit", "dsc", "darkwave-dex"];
function b64url(str) {
...
function hmac(data) {
  return crypto.createHmac("sha256", SSO_SECRET).update(data).digest("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/, "");
}
  ...
  if (urlPath === "/api/sso/exchange" && req.method === "POST") {
    ...
    try {
      ...
      if (userResult.rows.length > 0 && userResult.rows[0].pin_hash) {
        ...
        if (u.pin_locked_until && new Date(u.pin_locked_until) > /* @__PURE__ */ new Date()) {
          await pool.end();
        ...
        if (!verifyPinStr(pinProvided, u.pin_hash)) {
          ...
          const lockUntil = attempts >= PIN_MAX_ATTEMPTS ? new Date(Date.now() + PIN_LOCKOUT_MINUTES * 6e4).toISOString() : null;
...
function hashPin(pin) {
  ...
  const hash = crypto.pbkdf2Sync(pin, salt, 1e5, 64, "sha512").toString("hex");
  return `${salt}:${hash}`;
}
function verifyPin(pin, stored) {
  const [salt, hash] = stored.split(":");
  if (!salt || !hash) return false;
  const check = crypto.pbkdf2Sync(pin, salt, 1e5, 64, "sha512").toString("hex");
  return check === hash;
}
var verifyPinStr = verifyPin;
async function getDbPool() {
  const dbUrl = process.env.DATABASE_URL;
  if (!dbUrl) return null;
  const pgModule = await import("pg");
  const PoolClass = pgModule.Pool || pgModule.default?.Pool || pgModule.default;
  return new PoolClass({ connectionString: dbUrl });
}
async function handlePinRequest(req, res, urlPath) {
  if (urlPath === "/api/pin/set" && req.method === "POST") {
    const body = await readBody(req);
    const { email, pin, currentPin, ssoToken } = body;
    if (!email || !pin) {
      jsonResponse(res, 400, { success: false, error: "email and pin required" });
      return;
    }
    const appSecret = req.headers["x-darkwave-secret"];
    ...
    if (!validation.valid) {
      jsonResponse(res, 400, { success: false, error: validation.error });
      return;
    }
    const pool = await getDbPool();
    if (!pool) {
      jsonResponse(res, 503, { success: false, error: "Database not available" });
      return;
    }
    try {
      ...
      if (user.rows.length === 0) {
        await pool.end();
        jsonResponse(res, 404, { success: false, error: "User not found" });
        return;
      }
      if (user.rows[0].pin_hash) {
        if (!currentPin) {
          await pool.end();
          jsonResponse(res, 400, { success: false, error: "Current PIN required to change PIN" });
          return;
        }
        if (!verifyPin(currentPin, user.rows[0].pin_hash)) {
          await pool.end();
          jsonResponse(res, 401, { success: false, error: "Current PIN incorrect" });
          return;
        }
      }
  ...
  if (urlPath === "/api/pin/verify" && req.method === "POST") {
    ...
    try {
      ...
      if (!u.pin_hash) {
        await pool.end();
        jsonResponse(res, 400, { success: false, error: "No PIN set for this user" });
        return;
      }
      if (u.pin_locked_until && new Date(u.pin_locked_until) > /* @__PURE__ */ new Date()) {
        const remaining = Math.ceil((new Date(u.pin_locked_until).getTime() - Date.now()) / 6e4);
        await pool.end();
  ...
  if (urlPath === "/api/pin/status" && req.method === "POST") {
    ...
    const { email } = body;
    if (!email) {
      jsonResponse(res, 400, { success: false, error: "email required" });
      return;
    }
    const pool = await getDbPool();
    if (!pool) {
    ...
    try {
      ...
      const u = user.rows[0];
      const isLocked = u.pin_locked_until && new Date(u.pin_locked_until) > /* @__PURE__ */ new Date();
      jsonResponse(res, 200, {
        success: true,
        hasPin: !!u.pin_hash,
  ...
  if (urlPath === "/api/pin/validate-rules" && req.method === "POST") {
    ...
    const { pin } = body;
    if (!pin) {
      jsonResponse(res, 400, { success: false, error: "pin required" });
      return;
    }
    const validation = validatePinRules(pin);
    jsonResponse(res, 200, {
  ...
  if (urlPath === "/api/ecosystem/widget-data" && req.method === "GET") {
    ...
    if (pool) {
      try {
        ...
        const mappedApps = (dbApps.rows || []).map((app) => ({
          id: app.id,
...
var ORBIT_BASE_URL = "https://orbitstaffing.io";
var ORBIT_REGISTER_URL = `${ORBIT_BASE_URL}/api/admin/ecosystem/register-app`;
var ORBIT_SSO_LOGIN_URL = `${ORBIT_BASE_URL}/api/auth/ecosystem-login`;
var ORBIT_AUTH_REGISTER_URL = `${ORBIT_BASE_URL}/api/chat/auth/register`;
var orbitRegistered = false;
var orbitRegistrationData = null;
...
async function orbitSsoLogin(identifier, credential) {
  try {
...
async function orbitAuthRegister(username, email, password, displayName) {
  try {
...
function handleOrbitBridgeRequest(req, res, urlPath) {
...
function startInngestDevServer() {
  ...
  inngestProcess.on("exit", (code) => {
    ...
    if (code !== 0) {
      ...
      setTimeout(startInngestDevServer, 1e4);
    }
  });
...
async function createGenesisHallmark() {
  try {
    ...
    const blockHeight = (1e6 + Math.floor(Math.random() * 9e6)).toString();
    await pool.query(
...
async function handleHallmarkRequest(req, res, urlPath) {
  try {
...
function getUserTier(convertedCount) {
...
async function getOrCreateUniqueHash(pool, userId) {
  const existing = await pool.query(
...
async function createTrustStampDirect(pool, userId, category, data) {
  const stampData = { ...data, appContext: "pulse", timestamp: (/* @__PURE__ */ new Date()).toISOString() };
...
async function handleAffiliateRequest(req, res, urlPath) {
  try {
    ...
    if (urlPath === "/api/affiliate/request-payout" && req.method === "POST") {
      ...
      const totalPending = pending.rows.reduce((sum, r) => sum + parseFloat(r.amount), 0);
      if (totalPending < 10) {
        await pool.end();
        jsonResponse(res, 400, { error: "Minimum payout is 10 SIG", currentPending: totalPending });
        return;
      }
      const commissionIds = pending.rows.map((r) => r.id);
      await pool.query(
