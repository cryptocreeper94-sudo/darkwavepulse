...
import { c as coinGeckoClient } from '../coinGeckoClient.mjs';
import 'fs';
import 'path';
...
async function fetchCryptoOverview(category, logger) {
  ...
  try {
    ...
    if (category === "gainers") {
      data = data.filter((coin) => coin.price_change_percentage_24h > 0).sort((a, b) => (b.price_change_percentage_24h || 0) - (a.price_change_percentage_24h || 0)).slice(0, 10);
    } else if (category === "losers") {
      data = data.filter((coin) => coin.price_change_percentage_24h < 0).sort((a, b) => (a.price_change_percentage_24h || 0) - (b.price_change_percentage_24h || 0)).slice(0, 10);
    }
    marketCache.set(cacheKey, {
      data,
      timestamp: Date.now()
...
//# sourceMappingURL=0cc49f17-597c-4021-92ca-63e534bad31a.mjs.map
