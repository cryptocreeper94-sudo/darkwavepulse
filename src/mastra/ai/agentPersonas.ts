export type AgentPersonaId = 
  ...
  | 'tyrone' | 'raj' | 'connor' | 'hassan' | 'javier' | 'kai'
  | 'zara' | 'maya' | 'chloe' | 'aaliyah' | 'sofia' | 'mei'
  | 'darnell' | 'vikram' | 'brad' | 'omar' | 'carlos' | 'hiroshi'
  | 'destiny' | 'aisha' | 'rachel' | 'deepika' | 'maria' | 'sakura'
  | 'clarence' | 'arjun' | 'richard' | 'ahmad' | 'roberto' | 'takeshi'
  | 'dorothy' | 'kamala' | 'margaret' | 'layla' | 'carmen' | 'keiko';
...
export function getAgentsByAge(age: 'young' | 'middle' | 'senior'): AgentPersona[] {
  return Object.values(agentPersonas).filter(p => p.age === age);
}

export function getAgentsByGender(gender: 'male' | 'female'): AgentPersona[] {
  return Object.values(agentPersonas).filter(p => p.gender === gender);
}

export function getAgentsByRace(race: string): AgentPersona[] {
  return Object.values(agentPersonas).filter(p => p.race.toLowerCase().includes(race.toLowerCase()));
}
...
${persona.tradingStyle === 'swing' ? 
  '- Capture multi-day moves\n- Patience for setups\n- Let winners ride' : ''}
${persona.tradingStyle === 'scalper' ? 
